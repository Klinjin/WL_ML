...
class Probability:
    ...
    @jit
    def bounded_variable_ln_dtheta_dx(x):
        # Log of the Jacobian transformation
        log_jacobian = jax.nn.log_sigmoid(x) + jnp.log(1.0 - jax.nn.sigmoid(x)) 
    ...
    def x_to_bounded_theta(x, theta_ranges):
        ...
        for i in range(n_params):
            theta = theta.at[i].set(Probability._x_to_bounded_theta(_x[i], theta_ranges[i]))
